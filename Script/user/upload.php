...
try {
  ...
  $stmt = $conn->prepare("SELECT * FROM tbl_fuel ORDER BY fuel");
  $stmt->execute();
  $result = $stmt->fetchAll();

    foreach($result as $row)
    {
    print '<option value="'.$row['fuel'].'">'.$row['fuel'].'</option>';
  }
...
<div class="form-group mb-3">
<label class="control-label">Short Description (108 Characters; This must include any additional features of the vehicle)</label>
<textarea maxlength="108" id="minle" class="form-control input-md" name="shortdesc" required></textarea>
</div>


<div class="form-group md-3">
  <label class="control-label">Description (Must include: Engine capacity, model, model year, transmission, mileage)</label>
